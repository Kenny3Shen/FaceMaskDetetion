            ...
            while True:
                ...
                if self.is_continue:
                    ...
                    if count % 30 == 0 and count >= 30:
                        ...
                        self.send_fps.emit('{}FPS'.format(fps))
                        start_time = time.time()
                    if self.vid_cap:
...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(self, parent=None):
        ...
        self.thinSpinBox.valueChanged.connect(lambda x: self.change_val(x, 'thinSpinBox'))
        self.thinSlider.valueChanged.connect(lambda x: self.change_val(x, 'thinSlider'))
    ...
    def load_setting(self):
        ...
        self.thinSpinBox.setValue(thin)
    ...
    def change_val(self, x, flag):
        ...
        elif flag == 'thinSpinBox':
            self.thinSlider.setValue(x)
        elif flag == 'thinSlider':
            self.thinSpinBox.setValue(x)
            self.det_thread.line_thickness = x
        else:
            pass
    ...
    def change_model(self, x):
        ...
        self.statistic_msg('检测模型切换为%s' % x)
    ...
    def open_file(self):
        ...
        if name:
            ...
            self.statistic_msg('已加载文件：{}，请点击开始按钮'.format(os.path.basename(name)))
    ...
    def show_statistic(self, statistic_dic):
        try:
            ...
            for i in statistic_dic:
                ...
                total = total + i[1]
            results.insert(0, "Total：{}".format(total))
    ...
    def closeEvent(self, event):
        ...
        config['thin'] = self.thinSpinBox.value()
        ...
        MessageBox(
            self.closeButton, title='关闭中', text='请稍等，正在保存配置文件', time=2000, auto=True).exec_()
        sys.exit(0)
